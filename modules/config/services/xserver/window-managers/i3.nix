...
in lib.mkMerge [
  ...
  (lib.mkIf cfg.enable {
    home-files.allUsers.".config/i3/config" = ''
      exec --no-startup-id "alttab -w 1 -d 0 -dk Delete -t 104x104 -i 104x48 -s 1 -bg '#0f1117' -fg '#c6c8d1' -frame '#242940' -inact '#1e2132' -font 'xft:monospace-10' -b 0 >/dev/null 2>&1"
